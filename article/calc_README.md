# 咕咕镇计算器
#### 作者：brutelor  
[咕咕镇计算器0.5.0.6版](../src/app/newkf0.5.0.6.rar)   
...
#### 可用的PC类型名
`MO`(默)、`LIN`(琳)、`AI`(艾)、`MENG`(梦)、`WEI`(薇)、`YI`(伊)、`MING`(冥)、`MIN`(命)、`WU`(舞)、`XI`(希)、`XIA`(霞)
#### 可用的NPC类型名
...
```
02/18 (0.5.0.6)
- 对应2/18咕咕镇更新，增加新角色霞，对应名称XIA。霞的计算细节目前还存在下面这些疑点有待验证：
  - 计算“35%对方物理防御”的附加魔穿时，对方的防御是否包含CI、舞的被动等因素增加的防御？目前出于计算顺序的原因，攻击方不包含，防守方包含。
  - 增加的附加魔穿是否能被破魔强化？目前认为可以。
  - 计算“自身魔法攻击高于对方魔法攻击”时，是否包含启程之风增加的魔法攻击？
  - 魔法攻击提升30%时是否能将启程之风的攻击力增加30%？目前认为可以。已确认等级挑战的魔攻会被提升30%。
- 希的生命值加成改为成长值*1
- 结果中的平局场数D=x的后面增加平局场数占总场数的百分比的显示。
02/09 (0.5.0.5)
- 修改六边形战士的基础技能率和暴击率。当选中天赋HONG时，一半概率为高技能率低暴击率，一半概率为低技能率高暴击率。高技能暴击率目前设置为等级*4，低技能暴击率为等级*1。
02/09 (0.5.0.4)
- 修正点数输出的结果中没有包含自身的争夺等级的问题。
02/09 (0.5.0.3)
- 对多个对手计算最终的平均胜率时(bpc、bnpc、rank等)，改为将各个对手的非平场次率额外乘到权重上。简单理解就是如果没有其他额外的权重因素且对每个对手测试次数相同，那么最终的平均胜率就等于 总胜场数/总非平场数。当然总非平场数为0的特殊情况下最终的平均胜率还是会输出50%。
02/08 (0.5.0.2)
